...
- Entities are registered per-module via `TypeOrmModule.forFeature([...])`; a few modules register _other_ modules' entities **read-only** (e.g. `admin` reads `Ride`/profiles/`Transaction`).
