...
export default function AuditPage() {
  ...
  const columns: Column<AuditEntry>[] = [
    ...
    { key: 'actor', header: 'Admin', render: (a) => a.actorEmail ?? a.actorId?.slice(0, 8) ?? '—' },
    {
      key: 'role',
      header: 'Role',
      render: (a) => (a.actorRole ? <Badge tone="brand">{a.actorRole}</Badge> : '—'),
    },
    {
      key: 'action',
      header: 'Action',
      render: (a) => <span className="font-mono text-xs text-white">{a.action}</span>,
    },
    { key: 'target', header: 'Target', render: (a) => (a.targetId ? a.targetId.slice(0, 8) : '—') },
    {
      ...
      render: (a) =>
        a.meta ? (
          <span className="font-mono text-xs text-subtle">{JSON.stringify(a.meta)}</span>
        ) : (
          '—'
        ),
    },
